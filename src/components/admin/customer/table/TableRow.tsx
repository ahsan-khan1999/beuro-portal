      ...
      {currentPageRows?.map((item, index) => {
        return (
          ...
          >
            ...
            <span className="break-all xs:flex mlg:hidden xlg:hidden maxSize:flex py-4  items-center">
              {item?.email}
            </span>
            <span className="py-4  flex items-center">
              {item?.company?.plan}
            ...
            <span
              className="col-start-9 col-end-10 py-4 flex justify-center items-center  rounded-md"
              onClick={() =>
                router.push({
