    ...
    <div>
      {dataToAdd?.map((item, index: number) => {
        return (
          ...
          >
            <span className="py-4 rounded-md flex items-center">{item.id?.slice(0, 5)}</span>
            ...
            <span className="py-4 flex justify-center items-center">
              <div
                className={`bg-[${item?.viewedAt ? "#45C769" : "#FF376F"}] text-white px-2 py-1 text-center rounded-md  w-[70px] text-sm`}
              >
                {item.status}
              </div>
            </span>
