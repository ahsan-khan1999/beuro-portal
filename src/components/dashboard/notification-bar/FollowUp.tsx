...
import { formatDateTimeToDate, formatDateTimeToTime, getDaysDifference } from "@/utils/utility";

export const FollowUpNotificationBar = ({ dashboard }: { dashboard: Dashboard | null }) => {
  const followUp = dashboard?.["FollowUp"]?.map((item) => ({
    title: item?.title, time: formatDateTimeToTime(item?.dateTime), date: formatDateTimeToDate(item?.dateTime), id: item?.refID, day: getDaysDifference(item?.dateTime), svg: `<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" viewBox="0 0 25 25" fill="none">
