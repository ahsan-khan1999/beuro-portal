...
import { MobileTabArrayTypes } from "@/types";
...
import { LeadMobileAdditionalDetail } from "./mobile-additional-detail";
...
export interface ServiceMobileDetailProps {
  label: string;
  value: string | ContentTableRowTypes | null;
}

export const LeadsMobileDetailData = ({
  loading,
  leadDetails,
  ...
  const serviceDetail: ServiceMobileDetailProps[] = [
    ...
    {
      label: `${translate("leads.service_details.desire_date")}`,
      value: formatDateTimeToDate(leadDetails?.desireDate),
    },
    {
    ...
    },
    {
      ...
      value: leadDetails?.contactAvailability,
    },
    {
      label: `${translate("leads.service_details.prefer_contact")}`,
      value: leadDetails?.preferredContact,
    },
    {
      label: `${translate("leads.service_details.lead_source")}`,
      value: leadDetails?.leadSource,
    },
  ];

  const renderComponent = (type: ComponentsType) => {
    switch (type) {
      case ComponentsType.customer:
        return <LeadMobileCustomerDetail data={leadDetails?.customerDetail} />;
      case ComponentsType.address:
        return <LeadsAddressMobileDetails address={addressDetail} />;
      case ComponentsType.service:
  ...
  const tabSection: MobileTabArrayTypes[] = [
    {
      icon: `<svg
      className="mr-3"
          ...
          {tabSection.map((item, index) => (
            <React.Fragment key={index}>
              <MobileDetailTab
                ...
                setTabType={() =>
                  setTabType(tabType === item.type ? null : item.type)
                }
                tabType={tabType ?? -1}
                heading={item.name}
                icon={item.icon}
