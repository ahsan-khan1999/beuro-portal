...
const InvoiceSection = ({ systemSetting, setSystemSetting }: { systemSetting: SystemSettingDataProps, setSystemSetting: SetStateAction<any> }) => {
  ...
  const toggleOverdue = () => {
    setSystemSetting({ ...systemSetting, "isInvoiceOverDue": !systemSetting.isInvoiceOverDue })
  };
  ...
  return (
    <SettingLayout>
      <div className="mb-4">
        ...
        <div
          className={`border rounded-md py-4 px-[18px] flex justify-between items-center w-full cursor-pointer ${systemSetting?.isInvoiceOverDue ? "border-[#4A13E7]" : ""
            }`}
          onClick={toggleOverdue}
        >
          <span
            className={`text-base font-medium ${systemSetting?.isInvoiceOverDue ? "text-[#4A13E7]" : ""
              }`}
          >
        ...
        {systemSetting?.isInvoiceOverDue && (
          <div className="mt-[22px] ">
            ...
            <DropDown
              items={Object.keys(staticEnums["InvoiceOverDueLimit"]).map((item) => ({ item: item }))}
