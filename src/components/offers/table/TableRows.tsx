...
import { getEmailColor, getOfferStatusColor, getPaymentTypeColor } from "@/utils/utility";
      ...
      {dataToAdd?.map((item, index) => {
        return (
          ...
          >
            ...
            <span className="py-4 ">
              <div
                className={`bg-[${getEmailColor(item.emailStatus)}] text-white px-2 py-1 text-center rounded-md  w-[70px] text-sm`}
              >
                {item.emailStatus}
              </div>
            </span>
