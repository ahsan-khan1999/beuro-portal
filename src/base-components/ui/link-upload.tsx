  ...
  return (
    <div className="mt-[10px]">
      <div className="flex items-center gap-x-4 mb-[10px]">
        <form
          className="w-full"
          onSubmit={(e) => (validation?.isValid && onAddLink(e)) || test()}
        >
          <input
            type="text"
            value={inputLink}
      ...
      {!validation?.isValid && (
        <span className="text-red text-sm">{validation?.message}</span>
      )}
      {enteredLinks?.map((link, index) => (
        ...
        >
          <Link
            href={link}
            target="_blank"
            className="text-[#0000EE] text-base font-normal underline truncate max-w-[500px]"
          >
            {link}
          </Link>
